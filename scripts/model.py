...
rec_enc = df_data.withColumn("is_recommended_enc", encode_is_recommended(F.col("is_recommended")))

# Evaluation functions

map_evaluator = RankingEvaluator(labelCol="initial_recommendations", predictionCol="prediction_recommendations", metricName="meanAveragePrecision")
ndcg_evaluator = RankingEvaluator(labelCol="initial_recommendations", predictionCol="prediction_recommendations", metricName="ndcgAtK", k=5)


def getPreferencesList(s, k=5):
    without_unliked = filter(lambda x : x[1] > 1, s)
    res = list(map(lambda x: x[0], sorted(without_unliked, key=lambda x: x[1], reverse=True)))
    if k is None:
        return res
    return res[:k]

preferencesListUDF = F.udf(lambda x: getPreferencesList(x, None), ArrayType(DoubleType()))
fivePreferencesListUDF = F.udf(lambda x: getPreferencesList(x, 5), ArrayType(DoubleType()))
...
def get_preferenced_data(prediction_df):
    initial_recs = get_preferences(prediction_df, in_column="is_recommended_enc", out_column="initial_recommendations")
    predicted_recs = get_preferences(prediction_df, in_column="prediction", out_column="prediction_recommendations", custom_udf = fivePreferencesListUDF)
...
def evaluate_recommendations(prediction_df, model_name = ""):
    ...
    transformed_data = get_preferenced_data(prediction_df)
...
als_train_data, als_test_data = als_data.randomSplit([.7, .3], seed = 42)

als_model = ALS(userCol = "user_id", itemCol = "app_id", ratingCol = "is_recommended_enc", seed = 42, nonnegative = True, coldStartStrategy = "drop")
als_params = ParamGridBuilder().addGrid(als_model.regParam, [0.05, 0.1]).addGrid(als_model.rank, [5, 10]).build()
cv_als = CrossValidator(estimator = als_model, estimatorParamMaps = als_params, evaluator = rmse_evaluator, parallelism = 2, numFolds = 2, seed = 42)
cv_als_model = cv_als.fit(als_train_data)


als_params_mapped = list(map(lambda x: dict(map(lambda y: (y[0].name, y[1]), x.items())) , als_params))
als_param_names = list(als_params_mapped[0].keys())
cv_als_config = list(map(lambda x: [float(x[name]) for name in als_param_names], als_params_mapped))
cv_als_config_df = spark.createDataFrame(data=cv_als_config, schema = als_param_names)
cv_als_config_df.show()
cv_als_config_df.coalesce(1)\
    .write\
    .mode("overwrite")\
    .format("csv")\
    .option("sep", ",")\
    .option("header", "true")\
    .csv("/project/pda/cv_als_config")
...
best_als_params_df = spark.createDataFrame(data=best_als_params, schema = ["parameter", "value"])
best_als_params_df.show()
best_als_params_df.coalesce(1)\
    .write\
    .mode("overwrite")\
    .format("csv")\
    .option("sep", ",")\
    .option("header", "true")\
    .csv("/project/pda/best_als_params")
...
als_recommendations, als_map_score, als_ndcg_score = evaluate_recommendations(
	als_predictions.withColumn("is_recommended_enc", F.col("is_recommended_enc").cast("double")), "ALS")


best_als_scores = [("MAP", float(als_map_score)), ("NDCG", float(als_ndcg_score))]
best_als_scores_df = spark.createDataFrame(data=best_als_scores, schema = ["metric", "value"])
best_als_scores_df.show()
best_als_scores_df.coalesce(1)\
    .write\
    .mode("overwrite")\
    .format("csv")\
    .option("sep", ",")\
    .option("header", "true")\
    .csv("/project/pda/best_als_scores")


als_recommendations.limit(SAVE_LIMIT).coalesce(1)\
    .write\
    .mode("overwrite")\
    .format("json")\
...
df_games = df_games.withColumn('linux', F.col('linux').cast('double')).withColumn('mac', F.col('mac').cast('double')).withColumn('win', F.col('win').cast('double'))
...
df_games_rec = df_games.join(rec_enc.select('is_recommended_enc', 'app_id', 'user_id'), "app_id", "inner")
df_games_rec = df_games_rec.withColumn("is_recommended_enc", F.col("is_recommended_enc").cast('double')-1.0)


feature_columns_rf = ["app_id", "positive_ratio", "user_reviews", "price_original", "discount", "rating", "linux", "mac", "win", "year", "user_id"]
vector_assembler = VectorAssembler(inputCols = feature_columns_rf, outputCol="features_unscaled")
...
rf_features_df.show()
rf_features_df.coalesce(1)\
    .write\
    .mode("overwrite")\
    .format("csv")\
    .option("sep", ",")\
    .option("header", "true")\
    .csv("/project/pda/rf_features")
...
## Cross-validation
rf_train_data, rf_test_data = rf_data.randomSplit([.7, .3], seed = 42)

rf_model = RandomForestClassifier(labelCol="is_recommended_enc", seed = 42)
rf_params = ParamGridBuilder().addGrid(rf_model.numTrees, [5, 10]).addGrid(rf_model.maxDepth, [3, 5]).build()
cv_rf = CrossValidator(estimator = rf_model, estimatorParamMaps = rf_params, evaluator = rmse_evaluator, parallelism = 2, numFolds = 2, seed = 42)
cv_rf_model = cv_rf.fit(rf_train_data)


rf_params_mapped = list(map(lambda x: dict(map(lambda y: (y[0].name, y[1]), x.items())) , rf_params))
rf_param_names = list(rf_params_mapped[0].keys())
cv_rf_config = list(map(lambda x: [float(x[name]) for name in rf_param_names], rf_params_mapped))
cv_rf_config_df = spark.createDataFrame(data=cv_rf_config, schema = rf_param_names)
cv_rf_config_df.show()
cv_rf_config_df.coalesce(1)\
    .write\
    .mode("overwrite")\
    .format("csv")\
    .option("sep", ",")\
    .option("header", "true")\
    .csv("/project/pda/cv_rf_config")
...
best_rf_params_df = spark.createDataFrame(data=best_rf_params, schema = ["parameter", "value"])
best_rf_params_df.show()
best_rf_params_df.coalesce(1)\
    .write\
    .mode("overwrite")\
    .format("csv")\
    .option("sep", ",")\
    .option("header", "true")\
    .csv("/project/pda/best_rf_params")
...
rf_predictions = final_rf.transform(rf_test_data)\
...
rf_recommendations, rf_map_score, rf_ndcg_score = evaluate_recommendations(rf_predictions.withColumn("is_recommended_enc", F.col("is_recommended_enc").cast("double")), "RF")

best_rf_scores = [("MAP", float(rf_map_score)), ("NDCG", float(rf_ndcg_score))]
best_rf_scores_df = spark.createDataFrame(data=best_rf_scores, schema = ["metric", "value"])
best_rf_scores_df.show()
best_rf_scores_df.coalesce(1)\
    .write\
    .mode("overwrite")\
    .format("csv")\
    .option("sep", ",")\
    .option("header", "true")\
    .csv("/project/pda/best_rf_scores")


rf_recommendations.limit(SAVE_LIMIT).coalesce(1)\
    .write\
    .mode("overwrite")\
    .format("json")\
