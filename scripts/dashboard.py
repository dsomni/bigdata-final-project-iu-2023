...
st.write(q1)
st.markdown(
    "As we can see, all games support steam deck, so we can exclude it from further use in models. Moreover, the Windows platform seems to be the most supported."
)
...
st.write(q2)
st.markdown(
    "As we can see, there are no recommended games that would not support the Windows platform, which means that the Windows platform support parameter is one of the key features."
)

st.subheader("Q3")
st.markdown(
    "Correlation between number of products that user have and the number of hes/her reviews"
)
st.write(q3)
st.markdown(
    "As we can see, there is no correlation between number of games and reviews for users, which mean that such feature doesn't "
)
...
st.write(q4)
st.markdown(
    "That insight give us the idea that if game has review, then it's very probable, that it iis recommended."
)
...
st.write(q5)
st.markdown(
    "As we can see, in 1998 some strange things happened: from that time one game is extremely popular.  \nMoreover, due to COVID-19 lockdown number of reviews has a peak."
)
...
q5_chart1 = (
    ...
    .encode(
        x="published_year:N",
        y="reviews_number",
        tooltip=["published_year", "reviews_number"],
    )
    .interactive()
...
q5_chart2 = (
    ...
    .encode(
        ...
        tooltip=["published_year", "avg_reviews_per_game"],
    )
    .properties(width=1200, height=500)
    .interactive()
    .configure_point(size=200)
...
st.markdown(
    "- `NDCG` at 5 is a measure of how many of the first 5 recommended documents are in the set of true relevant documents averaged across all users. In contrast to precision at 5, this metric takes into account the order of the recommendations (documents are assumed to be in order of decreasing relevance)."
)
st.markdown(
    "As you can see, despite the ALS is the purely recommendation model, the Random Forest showed better performance on both metrics."
)

st.markdown(
    "The differense is ~6%. That is not a big surprise, since our recommendation score has only 2 values (is recommended or not). Thus, RF model with greater number of significant features shows better performance. However, ALS model uses only 3 columns of information and none of it is from **games table**."
)
